 ...
 const handleAddSuburb = async () => {
  ...
  try {
    ...
    setSuburbsData((prev) => {
      ...
      return [...prev, { name, active: false, availableDays: [] }];
    });
  } catch (err) {
    console.error(err);
...
if (isInstructorLoading || isSuburbsLocations) return <LoadingSpinner />;
  ...
  return (
    <div className="min-h-screen">
     <div className="sticky top-0 left-0 z-99 bg-white py-3 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <PrimaryBtn onClick={handleSave} className="justify-center text-sm! md:text-base">
            Update All
          </PrimaryBtn>
        </div>
      </div>
      ...
      <td className="py-4 px-6">
        <div className="flex flex-wrap gap-1">
          {dayShort.map((day) => (
            <span
              key={day.key}
              onClick={() => toggleDayByName(suburb.name, day.label)}
