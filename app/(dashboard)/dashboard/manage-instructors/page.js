...
export default function ManageInstructors() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({email, status}) => {
      await axios.patch("/api/users", {email, status});
      const res = await axios.patch("/api/instructors", {email, status});
      return res.data;
    },
    onSuccess: async () => {
        ...
        <Modal onClose={() => setSelectedInstructor(null)}>
          <div className="space-y-6 text-sm">
            ...
            <div className="flex items-center gap-4">
              <Image
                src={selectedInstructor.photo || avatarImg}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedInstructor.areas?.length ? (
                  selectedInstructor.areas.map((area) => (
                    <span
                      key={area}
