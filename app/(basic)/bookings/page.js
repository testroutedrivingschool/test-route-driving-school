...
const { data: slots = [], isLoading: slotsLoading } = useQuery({
  queryKey: ["instructorSlots", selectedInstructor?._id, weekStart, weekEnd, selectedLocations],
  enabled: !!selectedInstructor && !!selectedLocations,
  queryFn: async () => {
    const res = await axios.get("/api/instructor-slots", {
  params: { instructorId: selectedInstructor._id, from: weekStart, to: weekEnd }
});
      return res.data;
...
const { data: bookings = [], isLoading: bookingsLoading } = useQuery({
  ...
  queryFn: async () => {
    const res = await axios.get(`/api/bookings?email=${selectedInstructor.email}`);
...
{!selectedInstructor ? (
  ...
  >
    <table className="w-full min-w-150 md:min-w-[700px] border-separate border-spacing-0 table-fixed h-full">
      ...
      <th key={index} className="py-2 px-1 border border-border-color text-center text-xs md:text-sm font-medium text-gray-500 md:uppercase md:tracking-wider sticky top-0 z-20 bg-white">
   ...
   const topBorder = hourLine ? "border-t border-t-black border-r-gray-500! " : "border-t border-t-gray-500 border-dashed";
...
if (isBooked) {
  ...
  if (vis === "publicNote") {
    return (
      <td key={dayIndex} className={`${tdClass} align-stretch`}>
  ...
  if (vis === "public") {
    return (
      <td key={dayIndex} className={tdClass}>
        <button
          onClick={() => handleBookNow(time, dayIndex, slot)}
