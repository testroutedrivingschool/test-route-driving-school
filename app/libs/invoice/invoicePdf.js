...
function invoiceHtml(data, logoUrl) {
  ...
  let detailLine = "";
  if (isBooking && bookingDateText) {
    // For booking, only include booking date and time if available
    detailLine = bookingTime
      ? `${bookingDateText} at ${bookingTime}`
      : bookingDateText;
  } else if (isPurchase) {
    // For purchase, include address if it's available
    detailLine = data.address ? `at ${data.address}` : "";
  }

  const pendingLine = !isPaid ? "Payment pending." : "";

  // For purchases, calculate the total from packages
  const packageTotal =
    data.packages?.reduce((acc, pkg) => acc + pkg.lineTotal, 0) || 0;
  const discountAmount = Number(data.discountAmount || 0);
  const finalAmount = packageTotal - discountAmount;

  const method = String(safe(data.paymentMethod)).toLowerCase();
  ...
  const packagesHtml = isPurchase
    ...
    <tr>
      ...
      <td class="t-right">$${(pkg.unitPrice * pkg.quantity).toFixed(2)}</td>
        ...
        .join("")
    : ""; // Empty string for booking type, as no packages are needed

  const today = formatAUDate(new Date());

  // Booking invoice structure
  const bookingInvoice = `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <style>
...
<body>
  ...
  const purchaseInvoice = `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <style>
...
<body>
  <div class="page">
    ...
    <div class="split">
      ...
      <div class="box totals">
        <div class="grand">TOTAL&nbsp;&nbsp;&nbsp;&nbsp;$${packageTotal.toFixed(2)}</div>
        <div class="section">
          <span class="label">Discount:</span> $${discountAmount.toFixed(2)}
        </div>
        <div class="grand">Paid Amount:&nbsp;&nbsp;&nbsp;&nbsp;$${finalAmount.toFixed(2)}</div>
        <div class="paid">
          ${isPaid ? "PAID IN FULL" : "UNPAID"} <br/>
          <small>on ${today}</small>
        </div>
      </div>
    </div>
  ...
  // Return the correct structure based on booking or purchase
  return isBooking ? bookingInvoice : purchaseInvoice;
}
