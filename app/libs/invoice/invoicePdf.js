...
function invoiceHtml(data, logoUrl) {
  ...
  // Calculate the total from packages
  const packageTotal = data.packages.reduce((acc, pkg) => acc + pkg.lineTotal, 0);
  ...
  // Payment card line
  const paidByCardLine = isPaid && method === "card"
    ? `Paid by Card (${brand || "CARD"} •••• ${last4 || "----"})`
    : "";

  // Generate HTML for packages list
